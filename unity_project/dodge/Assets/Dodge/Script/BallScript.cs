...
public class BallScript : MonoBehaviour
{
    ...
    public void RandomBall()
    {
        ...
        float randAngle = Mathf.Atan2(Agent.transform.localPosition.z - transform.localPosition.z, Agent.transform.localPosition.x - transform.localPosition.x) + Random.Range(-ball_random, ball_random);
        float randomSpeed = ball_speed + Random.Range(-0.5f * ball_random, 0.5f * ball_random);

        Rigidbody rig = GetComponent<Rigidbody>();

        rig.velocity = new Vector3(randomSpeed * Mathf.Cos(randAngle), 0, randomSpeed * Mathf.Sin(randAngle));
    }
