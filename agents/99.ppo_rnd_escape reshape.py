...
# 파라미터 값 세팅 
state_image_size = [3, 32, 32]
state_obs_size = 1

dim1 = (floor((state_image_size[1] - 8)/4 + 1),floor((state_image_size[2] - 8)/4 + 1))
...
run_step = 1200000 if train_mode else 0
test_step = 10000
...
class PPONetwork(torch.nn.Module):
    def __init__(self, **kwargs):
        ...
        self.conv1 = torch.nn.Conv2d(in_channels=state_image_size[0], out_channels=16,
                                     kernel_size=8, stride=4)
        self.conv2 = torch.nn.Conv2d(in_channels=16, out_channels=32,
                                     kernel_size=4, stride=2)
        self.flat = torch.nn.Flatten()
        self.i = torch.nn.Linear(state_total_size, 512)
...
class RNDNetwork(torch.nn.Module):
    def __init__(self, **kwargs):
        ...
        self.conv2 = torch.nn.Conv2d(in_channels=16, out_channels=32,
                                     kernel_size=4, stride=2)
        self.flat = torch.nn.Flatten()
        self.i = torch.nn.Linear(state_total_size, 128)
...
class PPOAgent:
    ...
    def train_model(self):
        ...
        rnd_loss = torch.sum((prediction - target) ** 2, dim = 1)
        rnd_loss = rnd_loss.unsqueeze(dim = 1)
        ...
        with torch.no_grad():
            ...
            value_rnd = self.preditor_network(state)

            prob_old = pi_old.gather(1, action.long())

            _, next_value = self.network(next_state)
            value_next_rnd = self.preditor_network(next_state)

            delta = reward + (1 - done) * discount_factor * next_value - value
            delta_rnd = rnd_loss + (1 - done) * discount_factor * value_next_rnd - value_rnd
            
            adv, adv_rnd = delta.clone(), delta_rnd.clone()
            adv, adv_rnd, done = map(lambda x: x.view(n_step, -1).transpose(0,1).contiguous(), [adv, adv_rnd, done])
            
            for t in reversed(range(n_step-1)):
                adv[:, t] += (1 - done[:, t]) * discount_factor * _lambda * adv[:, t+1]
                adv_rnd[:, t] += (1 - done[:, t]) * discount_factor * _lambda * adv_rnd[:, t+1]

            adv = adv.transpose(0,1).contiguous().view(-1, 1)
            adv_rnd = adv_rnd.transpose(0,1).contiguous().view(-1, 1)
            adv = adv + rnd_strength * adv_rnd

            ret = adv + value
            ret_rnd = adv_rnd + value_rnd
        ...
        for _ in range(n_epoch):
            ...
            for offset in range(0, len(reward), batch_size):
                ...
                _state, _action, _value_rnd, _ret, _ret_rnd, _adv, _prob_old =\
                    map(lambda x: x[idx], [state, action, value_rnd, ret, ret_rnd, adv, prob_old])
                ...
                critic_rnd_loss = F.mse_loss(_value_rnd, _ret_rnd).mean()

                #total_loss = actor_loss + critic_loss + critic_rnd_loss
        ...
        return np.mean(actor_losses), np.mean(critic_losses), np.mean(rnd_reward.item())
...
if __name__ == '__main__':
    ...
    for step in range(run_step + test_step):
        ...
        state = [[0 for _ in range(state_total_size)] for _ in range(num_worker)]
        action = [0 for _ in range(num_worker)]
        next_state = [[0 for _ in range(state_total_size)] for _ in range(num_worker)]
        ...
        for id in dec.agent_id:
            ...
            action_tuple.add_discrete(np.array([enum_act[tmpaction[0][0]]], dtype=int))
        ...
        for id in dec.agent_id:
            _id = list(dec.agent_id).index(id)
            next_state[id] = dec.obs[0]
            reward[id] = dec.reward[_id]
        for id in term.agent_id:
            _id = list(term.agent_id).index(id)
            done[id] = True
            next_state[id] = term.obs[0]
